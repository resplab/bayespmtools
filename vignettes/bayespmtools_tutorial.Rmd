...
We assume we plan to validate this study in the London region. Based on the exchengability assumption, the performance of the model in the London region will be a random draw from the predictive distribution of performance metrics observed across the other 8 regions of the UK.
...
```{r set-seed}
# Settings the random number seed for reproducibilty.
set.seed(123)
```
...
As can be seen above, different components require different sample size estimates. The largest sample size is XXX, dictated by on the assurance desired for the calibration slope criterion.
