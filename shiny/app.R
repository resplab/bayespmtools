...
ui <- fluidPage(
  ...
  # Application title
  titlePanel(
    "Bayesain power, assurance, and VoI calculator (CONFIDENTIAL - please do not share the link)"
  ),
  tabsetPanel(
    id = "input_tabset",
    tabPanel(
      ...
      selectInput(
        ...
        choices = c(
          "I know the sample size and want to estimate precision" = "pow",
    ...
    tabPanel(
      ...
      HTML(
        "Here we solicit your assessment of the model performance and your uncertainty around this asessment."
      ),
      hr(),

      sidebarPanel(
        "Evidence on outcome prevalence",
        selectInput(
      ...
      sidebarPanel(
        ...
        selectInput(
          "dist_type",
          "Distribution of calibrated risks",
          choices = c("logitnorm", "beta", "probitnorm")
        ),
      ),
      actionButton("btn_test_evidence", label = "Test evidence"),
      ...
      fluidRow(
        column(
          ...
          sidebarPanel(
            checkboxInput(
              "b_stats",
              "I want to investigate precision targets for statistical metrics (discrimination and calibration)",
              value = TRUE
            ),
            hr(),
            conditionalPanel(
              ...
              conditionalPanel(
                ...
                selectInput(
                  "feciw_type",
                  "Conventional or Bayesian?",
                  choices = c(
                    "Point estimate of CI width (conventional)" = "fciw",
                    "Expected CI width (Bayesian)" = "eciw"
                  )
                ),
              ),
              ...
              conditionalPanel(
                condition = "input.b_fciw+input.b_eciw+input.b_qciw==0",
                HTML(
                  "<font color='red'>At least one item needs to be selected.</font>"
                )
              ),
    ...
    tabPanel(
      ...
      sidebarPanel(
        textAreaInput(
          "N",
          "Please enter sample sizes of interest (comma separated)",
  ...
  tags$div(
    class = "footer",
    "App version 2025.01.28. For questions and bug reports contact msafavi@mail.ubc.ca"
  )
)
...
server <- function(input, output) {
  ...
  observeEvent(input$btn_test_evidence, {
    isolate({
      ...
      e2 <- express_evidence(process_evidence(e1))
    })
    output$test_evidence_results <- renderText(paste0(
  ...
  observeEvent(input$btn_run, {
    ...
    console <- capture.output(
      {
        res <- do.call(
          ifelse(input$purpose == "pow", bpm_valpow, bpm_valsamp),
          args = args
        )
      },
      type = "message"
