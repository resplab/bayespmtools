...
Specified evidence should be a list of formulas, with exact naming of parameters per below. Currently recognized distributions are norm (Normal), logitnorm(Logit-normal), probitnorm (Probit-normal), and beta (Beta). Note the flexibility in specifying the distributions. If paramters are not named, they are taken as the natural parameters for the distribution. Ohterwise they are internally mapped to the parameters of the distributin. For example, the mean and SD specified for prevalence are internally converted to the two parameters of the Beta distribution.

```{r load-evidence}
evidence <- list(
    prev~beta(mean=0.428, sd=0.03), 
    cstat~logitnorm (mean=0.761, sd=0.006), 
