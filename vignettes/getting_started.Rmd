...
```{r}
evidence <- list(
  prev ~ beta(mean = 0.428, sd = 0.030),     # Outcome prevalence
  cstat ~ beta(mean = 0.761, cih = 0.773),   # C-statistic
...
targets <- list(
  ...
  qciw.cal_slp = c(0.9, 0.35),  # 90% assurance that CI width ≤ 0.12
  voi.nb = 0.90
)
```
...
The output shows the required sample size for each criterion. The largest sample size (1035) ensures all targets are met. However, the VoI criterion indicates that a sample size of 717 is expected to reduce uncertainty-related clinical utility loss by 90%. This might be used to argue that, potentially, the criteria on calibration slope can be relaxed.
